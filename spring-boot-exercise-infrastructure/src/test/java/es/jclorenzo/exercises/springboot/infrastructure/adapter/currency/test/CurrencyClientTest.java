package es.jclorenzo.exercises.springboot.infrastructure.adapter.currency.test;
...
import es.jclorenzo.exercises.springboot.infrastructure.adapter.currency.CurrencyRemoteClientImpl;
import es.jclorenzo.exercises.springboot.infrastructure.adapter.currency.test.config.CurrencyClientTestConfiguration;
 ...
 */
@Slf4j
@SpringBootTest(classes = CurrencyClientTestConfiguration.class)
public class CurrencyClientTest {
	...
	public void findCurrenciesTest() {
		...
		Assertions.assertDoesNotThrow(() -> {
			final List<CurrencyVO> result = this.currencyClient.getCurrencies();
	...
	public void findCurrencyEuroTest() {
		...
		Assertions.assertDoesNotThrow(() -> {
			final CurrencyVO result = this.currencyClient.findByCode(currencyCode);
			Assertions.assertNotNull(result);
			Assertions.assertEquals(currencyCode, result.getCode());
		});
	}
	...
	public void notFoundError() {
		...
		Assertions.assertThrows(NoSuchElementException.class, () -> {
			this.currencyClient.findByCode(currencyCode);
		});
	}
